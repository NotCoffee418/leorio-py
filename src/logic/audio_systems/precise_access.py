...
def start_wakeword_detection(on_prediction, on_activation):
    ...
    runner = PreciseRunner(precise_engine, on_prediction=on_prediction, on_activation=on_activation, trigger_level=0)
    runner.start()
    runner.stop()
...
def initialize_precise():
    ...
    precise_engine = PreciseEngine(_get_engine_exe_path(), _get_wakeword_model_path())
